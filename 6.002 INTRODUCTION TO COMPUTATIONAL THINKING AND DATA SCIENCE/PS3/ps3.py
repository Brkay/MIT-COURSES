...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.height = height
        self.tile_dict = {}
        self.initial_dirt_amount = dirt_amount
        for i in range(width):
            for j in range(height):
                self.tile_dict[(i, j)] = dirt_amount
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        coorditanes = (math.floor(pos.get_x()), math.floor(pos.get_y()))
        pos_dirt = self.tile_dict[coorditanes]
        if pos_dirt >= capacity:
            self.tile_dict[coorditanes] = pos_dirt-capacity
        else:
            self.tile_dict[coorditanes] = 0
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        total_cleaned = 0
        for keys in self.tile_dict:
            if self.tile_dict[keys] == 0:
                total_cleaned += 1
    ...
    def is_position_in_room(self, pos):
        ...
        """
        coorditanes = (math.floor(pos.get_x()), math.floor(pos.get_y()))
        return coorditanes in self.tile_dict.keys()
...
class FurnishedRoom(RectangularRoom):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.valid_tiles = [(x, y) for x in range(self.get_width())
    ...
    def get_num_tiles(self):
        ...
        """
        number_of_tiles_furnished = len(self.furniture_tiles)
        total_number_of_tiles = self.get_height() * self.get_width()
        return total_number_of_tiles - number_of_tiles_furnished

    def get_random_position(self):
        """
        Returns: a Position object; a valid random position (inside the room and not in a furnished area).
        """
        temp_tile = self.valid_tiles[random.randint(
            0, len(self.valid_tiles)-1)]
        return Position(temp_tile[0], temp_tile[1])
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        temp_pos = self.get_robot_position()
        new_position = temp_pos.get_new_position(
            self.get_robot_direction(), self.get_robot_speed())
        if self.room.is_position_valid(new_position):
            self.room.clean_tile_at_position(new_position, self.capacitiy)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """
    time_steps_list = [0] * num_trials
    for index0 in range(num_trials):
        room_object = room_type(width, height, dirt_amount)
        if room_type == FurnishedRoom:
            room_object.add_furniture_to_room()
        anim = ps3_visualize.RobotVisualization(num_robots, width, height,
                                                room_type == FurnishedRoom, delay=0.5)
        total_tiles = room_object.get_num_tiles()
        robot_list = []
        for i in range(num_robots):
            robot_list.append(robot_type(
                room_object, speed, capacity))
        cleaned_tiles = room_object.get_num_cleaned_tiles()
        while(min_coverage > (cleaned_tiles / total_tiles)):
            for index2 in range(num_robots):
                robot_list[index2].update_position_and_clean()
                cleaned_tiles = room_object.get_num_cleaned_tiles()
                anim.update(room_object, robot_list)
                if min_coverage <= (cleaned_tiles / total_tiles):
                    break
            time_steps_list[index0] += 1
        anim.done()

    return sum(time_steps_list) / num_trials
...
#       of a 20x20 room?
""" print('avg time steps for standard robot: ' + str(run_simulation(1,
      1.0, 1, 20, 20, 3, 0.8, 50, StandardRobot)))
print('avg time steps for faulty robot: ' + str(run_simulation(3,
      1.0, 1, 20, 20, 3, 0.8, 50, FaultyRobot)))
...
width = 10
height = 30
print('avg time steps for standard robot when dimensions = (%d, %d): ' % (width, height) + str(run_simulation(1,
      1.0, 1, width, height, 3, 0.8, 50, StandardRobot)))
print('avg time steps for faulty robot when dimensions = (%d, %d): ' % (width, height) + str(run_simulation(3,
      1.0, 1, width, height, 3, 0.8, 50, FaultyRobot)))
width = 20
height = 15
print('avg time steps for standard robot when dimensions = (%d, %d): ' % (width, height) + str(run_simulation(1,
      1.0, 1, width, height, 3, 0.8, 50, StandardRobot)))
...
height = 8
print('avg time steps for faulty robot when dimensions = (%d, %d): ' % (width, height) + str(run_simulation(3,
      1.0, 1, width, height, 3, 0.8, 1, FaultyRobot, EmptyRoom)))

print('avg time steps for standard robot when dimensions = (%d, %d): ' % (width, height) + str(run_simulation(2,
      1.4, 1.2, width, height, 6, 0.8, 1, StandardRobot, FurnishedRoom)))
